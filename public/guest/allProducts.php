...
<section class="section">
  ...
  <div class="d-flex align-items-center mb-3 px-3 py-2 bg-white rounded-3 border"
    ...
    <form method="GET" action="" class="d-flex align-items-center gap-2 flex-shrink-0">
      ...
      <select name="sort" class="form-select form-select-sm border-0 fw-bold" style="width:auto;" onchange="this.form.submit()">
      ...
      <?php if ($search || $priceRange || $sort !== 'newest'): ?>
        <a href="allProducts" class="btn btn-sm btn-outline-secondary" title="Clear filters"><i class="bi bi-x-circle"></i></a>
      <?php endif; ?>
    </form>
  </div>
  ...
  <?php if ($productsResult->num_rows === 0): ?>
    <div class="card"><div class="card-body">
      <div class="empty-state">
        <i class="bi bi-bag-x"></i><h5>No products found</h5>
      ...
      <nav class="mt-4 d-flex justify-content-center">
        <ul class="pagination">
          <li class="page-item <?= $page<=1?'disabled':'' ?>">
            <a class="page-link" href="?page=<?=$page-1?>&search=<?=urlencode($search)?>&price=<?=urlencode($priceRange)?>&sort=<?=urlencode($sort)?>"><i class="bi bi-chevron-left"></i></a>
          </li>
          <?php for ($i=1; $i<=$totalPages; $i++): ?>
            <li class="page-item <?= $i===$page?'active':'' ?>">
              <a class="page-link" href="?page=<?=$i?>&search=<?=urlencode($search)?>&price=<?=urlencode($priceRange)?>&sort=<?=urlencode($sort)?>"><?=$i?></a>
            </li>
          <?php endfor; ?>
          <li class="page-item <?= $page>=$totalPages?'disabled':'' ?>">
            <a class="page-link" href="?page=<?=$page+1?>&search=<?=urlencode($search)?>&price=<?=urlencode($priceRange)?>&sort=<?=urlencode($sort)?>"><i class="bi bi-chevron-right"></i></a>
          </li>
        </ul>
      </nav>
    <?php endif; ?>
