...
document.getElementById('placeOrderBtn')?.addEventListener('click', () => {
  ...
  if (method === 'cod') {
    // COD: submit form directly, no modal needed
    const form = document.getElementById('checkoutForm');
    // Guard against duplicate hidden inputs on multiple clicks
    if (!form.querySelector('[name="placeOrder"]')) {
      const hidden = document.createElement('input');
      hidden.type  = 'hidden';
      hidden.name  = 'placeOrder';
      hidden.value = '1';
  ...
  // paymentInstructions and methodMetaMap are injected as <script> vars by checkout.php
  ...
  if (instructions?.note) {
    noteTextEl.innerHTML = instructions.note;
...
document.getElementById('confirmOrderBtn')?.addEventListener('click', () => {
  ...
  const form = document.getElementById('checkoutForm');
  // Guard against duplicate hidden inputs if user clicks Confirm multiple times
  if (!form.querySelector('[name="placeOrder"]')) {
    const hidden = document.createElement('input');
    hidden.type  = 'hidden';
    hidden.name  = 'placeOrder';
