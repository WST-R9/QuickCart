...
// ----------------------------------------
// CATEGORIES for filter pills (top-level only)
// ----------------------------------------
$catsResult = mysqli_query(
    $conn,
    "SELECT categoryId, name FROM categories WHERE parentId IS NULL ORDER BY name ASC"
);
...
if ($categoryId > 0) {
    // Match exact category OR its children
...
<section class="section">
    ...
    <form method="GET" action="" class="row g-2 align-items-center">
      ...
      <?php if ($search || $categoryId || $sort !== 'newest'): ?>
        <div class="col-12 col-md-2">
          <a href="allProducts" class="btn btn-outline-secondary w-100 btn-clear-filter">
  ...
  <div class="category-pills">
    <a href="allProducts?sort=<?= urlencode($sort) ?><?= $search ? '&search='.urlencode($search) : '' ?>"
       class="pill <?= $categoryId === 0 ? 'active' : '' ?>">
      All
    </a>
    <?php while ($cat = mysqli_fetch_assoc($catsResult)): ?>
      <a href="allProducts?category=<?= $cat['categoryId'] ?>&sort=<?= urlencode($sort) ?><?= $search ? '&search='.urlencode($search) : '' ?>"
         class="pill <?= $categoryId === (int)$cat['categoryId'] ? 'active' : '' ?>">
  ...
  <div class="results-meta">
    ...
    <?php if ($categoryId > 0): ?>
      <?php
        $r  = mysqli_query($conn, "SELECT name FROM categories WHERE categoryId = $categoryId");
        $cn = mysqli_fetch_assoc($r)['name'] ?? '';
      ?>
      in <strong><?= htmlspecialchars($cn) ?></strong>
    <?php endif; ?>
  </div>
      ...
      <nav class="mt-4 d-flex justify-content-center">
        <ul class="pagination">
          <li class="page-item <?= $page <= 1 ? 'disabled' : '' ?>">
            <a class="page-link" href="?page=<?= $page - 1 ?>&search=<?= urlencode($search) ?>&category=<?= $categoryId ?>&sort=<?= urlencode($sort) ?>">
          ...
          if ($start > 1):
          ?>
            <li class="page-item"><a class="page-link" href="?page=1&search=<?= urlencode($search) ?>&category=<?= $categoryId ?>&sort=<?= urlencode($sort) ?>">1</a></li>
            <?php if ($start > 2): ?><li class="page-item disabled"><span class="page-link">…</span></li><?php endif; ?>
          <?php endif; ?>

          <?php for ($i = $start; $i <= $end; $i++): ?>
            <li class="page-item <?= $i === $page ? 'active' : '' ?>">
              <a class="page-link" href="?page=<?= $i ?>&search=<?= urlencode($search) ?>&category=<?= $categoryId ?>&sort=<?= urlencode($sort) ?>"><?= $i ?></a>
            </li>
          <?php endfor; ?>

          <?php if ($end < $totalPages): ?>
            <?php if ($end < $totalPages - 1): ?><li class="page-item disabled"><span class="page-link">…</span></li><?php endif; ?>
            <li class="page-item"><a class="page-link" href="?page=<?= $totalPages ?>&search=<?= urlencode($search) ?>&category=<?= $categoryId ?>&sort=<?= urlencode($sort) ?>"><?= $totalPages ?></a></li>
          <?php endif; ?>
