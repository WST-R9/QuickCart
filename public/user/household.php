...
// ----------------------------------------
// GET HOUSEHOLD CATEGORY ID
// ----------------------------------------
$catResult = mysqli_query($conn, "SELECT categoryId, name FROM categories WHERE name = 'Household' LIMIT 1");
$category  = mysqli_fetch_assoc($catResult);
$catId     = $category['categoryId'] ?? null;
$catName   = $category['name'] ?? 'Household';
    ...
    <form method="GET" action="" class="row g-2 align-items-center">
      ...
      <?php if ($search || $sort !== 'newest'): ?>
        <div class="col-12 col-md-2">
          <a href="household" class="btn btn-outline-secondary w-100 btn-clear-filter">
    ...
    <div class="card">
      <div class="card-body">
        <div class="empty-state">
          ...
          <a href="household" class="btn btn-primary mt-2">
            <i class="bi bi-arrow-left me-1"></i> Back to Household
          </a>
        </div>
      </div>
    </div>
          ...
          <div class="product-card">
            ...
            <div class="product-body">
              <div class="product-category">
                <?= htmlspecialchars($product['categoryName'] ?? 'Household') ?>
              </div>
    ...
    <?php if ($totalPages > 1): ?>
      <nav class="mt-4 d-flex justify-content-center flex-wrap gap-2">
        <ul class="pagination">
          ...
          if ($start > 1): ?>
            <li class="page-item">
              <a class="page-link" href="?page=1&search=<?= urlencode($search) ?>&sort=<?= urlencode($sort) ?>">1</a>
            </li>
            <?php if ($start > 2): ?>
          ...
          <?php if ($end < $totalPages): ?>
            ...
            <?php endif; ?>
            <li class="page-item">
              <a class="page-link" href="?page=<?= $totalPages ?>&search=<?= urlencode($search) ?>&sort=<?= urlencode($sort) ?>"><?= $totalPages ?></a>
            </li>
          <?php endif; ?>
