...
if (isset($_POST['updateOrderStatus'])) {
  ...
  if (!in_array($newStatus, $valid)) {
    ...
    $_SESSION['code'] = "error";
    header("Location: ordersView?id=$orderId");
    exit;
  }
  ...
  if ($newStatus === 'confirmed') {
    ...
    if (mysqli_num_rows($chk) == 0) {
      ...
      if ($addr) {
        $rn = mysqli_real_escape_string($conn, $addr['recipientName']);
        $ph = mysqli_real_escape_string($conn, $addr['phoneNumber']);
        $st = mysqli_real_escape_string($conn, $addr['street']);
        $ba = mysqli_real_escape_string($conn, $addr['barangay']);
        $ci = mysqli_real_escape_string($conn, $addr['city']);
        $pr = mysqli_real_escape_string($conn, $addr['province'] ?? '');
        $zi = mysqli_real_escape_string($conn, $addr['zipCode'] ?? '');
        $ai = $addr['addressId'];
  ...
  if ($newStatus === 'shipped')
    mysqli_query($conn, "UPDATE shipping SET status='shipped',shippedAt=NOW() WHERE orderId=$orderId");
  if ($newStatus === 'delivered') {
    ...
    if ($pay && $pay['method'] === 'cod' && $pay['status'] === 'pending')
      mysqli_query($conn, "UPDATE payments SET status='paid',paidAt=NOW() WHERE orderId=$orderId");
  }
  if ($newStatus === 'cancelled') {
    $its = mysqli_query($conn, "SELECT productId,quantity FROM orderitems WHERE orderId=$orderId");
    while ($it = mysqli_fetch_assoc($its))
      if ($it['productId'])
        mysqli_query($conn, "UPDATE products SET stock=stock+{$it['quantity']} WHERE productId={$it['productId']}");
  ...
  if (isset($msgs[$newStatus]))
    createNotification($conn, $or['userId'], 'customer', 'order_status', "Order " . ucfirst($newStatus), $msgs[$newStatus], $orderId, 'order');
  $_SESSION['message'] = "Order updated to " . ucfirst($newStatus) . ".";
...
if (isset($_POST['updateShippingStatus'])) {
  ...
  if ($ns === 'shipped' || $ns === 'out_for_delivery')
  ...
  if (isset($smsgs[$ns]))
    createNotification($conn, $or['userId'], 'customer', 'order_status', ucfirst(str_replace('_', ' ', $ns)), $smsgs[$ns], $orderId, 'order');
  $_SESSION['message'] = "Shipping updated to " . ucfirst(str_replace('_', ' ', $ns)) . ".";
  $_SESSION['code'] = "success";
...
if (isset($_POST['updateShippingDetails'])) {
  ...
  if (empty($c) || empty($t)) {
    $_SESSION['message'] = "Courier and Tracking Number are required.";
    $_SESSION['code'] = "error";
    header("Location: ordersView?id=$orderId");
    exit;
  }
  mysqli_query($conn, "UPDATE shipping SET courier='$c',trackingNumber='$t' WHERE orderId=$orderId");
  $_SESSION['message'] = "Shipping details saved.";
  $_SESSION['code'] = "success";
  header("Location: ordersView?id=$orderId");
  exit;
}
...
if (isset($_POST['updatePaymentStatus'])) {
  ...
  $newPayStatus = mysqli_real_escape_string($conn, $_POST['paymentStatus']);
  $refNum = mysqli_real_escape_string($conn, trim($_POST['referenceNumber'] ?? ''));
  ...
  $paidAt = $newPayStatus === 'paid' ? ", paidAt=NOW()" : "";
  mysqli_query($conn, "UPDATE payments SET status='$newPayStatus', referenceNumber='$refNum' $paidAt WHERE paymentId=$paymentId AND orderId=$orderId");
  $_SESSION['message'] = "Payment updated to " . ucfirst($newPayStatus) . ".";
  $_SESSION['code'] = "success";
  header("Location: ordersView?id=$orderId");
  exit;
}
...
if (isset($_POST['uploadProof'])) {
  if (!empty($_FILES['proofImage']['tmp_name'])) {
    ...
    if (move_uploaded_file($_FILES['proofImage']['tmp_name'], $dir . $fn)) {
      ...
      $_SESSION['message'] = "Proof uploaded.";
      $_SESSION['code'] = "success";
    } else {
      ...
      <div class="card">
        <div class="card-body">
          ...
          <div class="row g-3">
            ...
            <div class="col-md-6">
              <p class="mb-1 text-muted small">Status</p><span
                class="badge <?= orderBadge($order['status']) ?>"><?= ucfirst($order['status']) ?></span>
            </div>
            <div class="col-md-6">
            ...
            <div class="col-md-6">
              ...
              <p><?= date("M d, Y h:i A", strtotime($order['orderedAt'])) ?></p>
            </div>
          ...
          <?php if (!in_array($order['status'], ['delivered', 'cancelled', 'refunded'])): ?>
            ...
            <form method="POST" class="d-flex align-items-center gap-2 flex-wrap">
              ...
              <button type="submit" name="updateOrderStatus" class="btn btn-primary btn-sm"><i
                  class="bi bi-check-circle me-1"></i>Update</button>
            </form>
          <?php else: ?>
            <hr>
            <p class="text-muted small mb-0"><i class="bi bi-lock me-1"></i>Order is
              <strong><?= ucfirst($order['status']) ?></strong> — locked.
            </p>
          <?php endif; ?>
        </div>
      </div>
          ...
          <div class="card-body">
            ...
            <div class="row g-2 mb-3">
              <div class="col-md-4"><span class="text-muted small">Status</span><br>
                ...
                <span class="badge <?= $rb ?>"><?= ucfirst($refundReq['status']) ?></span>
              </div>
              <div class="col-md-8"><span
                  class="text-muted small">Reason</span><br><strong><?= htmlspecialchars($refundReq['reason']) ?></strong>
              </div>
              <?php if ($refundReq['details']): ?>
                <div class="col-12"><span class="text-muted small">Details</span><br>
                  <p class="mb-0 small"><?= nl2br(htmlspecialchars($refundReq['details'])) ?></p>
                </div><?php endif; ?>
              <?php if ($refundReq['imageProof']): ?>
                <div class="col-12"><span class="text-muted small">Proof</span><br><img
                    src="../uploads/refunds/<?= htmlspecialchars($refundReq['imageProof']) ?>"
                    class="img-fluid rounded border mt-1" style="max-height:140px;"></div><?php endif; ?>
            </div>
            <a href="refundsView?id=<?= $refundReq['refundId'] ?>" class="btn btn-sm btn-warning"><i
                class="bi bi-eye me-1"></i>View &amp; Process Refund</a>
          </div>
        </div>
      <?php endif; ?>
          ...
          <div class="card-body">
            ...
            <?php if (!empty($shipping['proofOfDelivery'])): ?>
              <img src="../uploads/proof/<?= htmlspecialchars($shipping['proofOfDelivery']) ?>"
                class="img-fluid rounded border mb-2" style="max-height:280px;cursor:pointer;" data-bs-toggle="modal"
              ...
              <div class="modal fade" id="proofModal" tabindex="-1">
                <div class="modal-dialog modal-lg modal-dialog-centered">
                  <div class="modal-content">
                    <div class="modal-header border-0"><button type="button" class="btn-close"
                        data-bs-dismiss="modal"></button></div>
                    <div class="modal-body text-center"><img
                        src="../uploads/proof/<?= htmlspecialchars($shipping['proofOfDelivery']) ?>"
            ...
            <?php else: ?>
              <form method="POST" enctype="multipart/form-data" class="d-flex gap-2">
                <input type="file" name="proofImage" class="form-control form-control-sm" accept="image/*"
                  style="width:auto;" required>
                <button type="submit" name="uploadProof" class="btn btn-primary btn-sm"><i
                    class="bi bi-upload me-1"></i>Upload Proof</button>
              </form>
            <?php endif; ?>
          </div>
        </div>
    ...
    <div class="col-lg-4">
      <div class="card">
        <div class="card-body">
          ...
          <a href="customersView?id=<?= $order['userId'] ?>" class="btn btn-sm btn-primary"><i
              class="bi bi-person me-1"></i>View Customer</a>
        </div>
      </div>
      ...
      <div class="card">
        <div class="card-body">
          ...
          <?php if ($isCancelled): ?>
            <div
              class="alert alert-<?= $order['status'] === 'refunded' ? 'secondary' : 'warning' ?> py-2 px-3 small mb-2">
              Order
              is <strong><?= ucfirst($order['status']) ?></strong>.</div>
          <?php endif; ?>
          <?php if ($payment): ?>
            <p class="mb-1"><strong>Method:</strong> <?= strtoupper(str_replace('_', ' ', $payment['method'])) ?></p>
            <p class="mb-1"><strong>Status:</strong> <span
                class="badge <?= paymentBadge($payment['status']) ?>"><?= ucfirst($payment['status']) ?></span></p>
            <p class="mb-2"><strong>Amount:</strong> ₱<?= number_format($payment['amount'], 2) ?></p>
            <?php if (!in_array($payment['status'], ['paid', 'refunded']) && !$isCancelled): ?>
              <hr>
              <form method="POST" class="d-flex flex-column gap-2">
                <input type="hidden" name="paymentId" value="<?= $payment['paymentId'] ?>">
                <div>
                  <label class="form-label small fw-semibold mb-1">Update Payment Status</label>
                  <select name="paymentStatus" class="form-select form-select-sm">
                    <?php foreach (['pending', 'paid', 'failed', 'refunded'] as $ps): ?>
                      <option value="<?= $ps ?>" <?= $payment['status'] === $ps ? 'selected' : '' ?>><?= ucfirst($ps) ?></option>
                    <?php endforeach; ?>
                  </select>
                </div>
                <div>
                  <input type="text" name="referenceNumber" class="form-control form-control-sm"
                    placeholder="Reference # (optional)" value="<?= htmlspecialchars($payment['referenceNumber'] ?? '') ?>">
                </div>
                <button type="submit" name="updatePaymentStatus" class="btn btn-outline-success btn-sm">
                  <i class="bi bi-check-circle me-1"></i>Update Payment
                </button>
              </form>
            <?php else: ?>
              <p class="text-muted small mb-0"><i class="bi bi-lock me-1"></i>Payment is
                <strong><?= ucfirst($payment['status']) ?></strong> — locked.
              </p>
            <?php endif; ?>
          <?php else: ?>
            <p class="text-muted small mb-0">No payment record.</p><?php endif; ?>
        </div>
      </div>
        ...
        <div class="card-body">
          ...
          <?php if ($shipping): ?>
            <?php if ($isCancelled): ?>
              <div
                class="alert alert-<?= $order['status'] === 'refunded' ? 'secondary' : 'warning' ?> py-2 px-3 small mb-2">
                Shipment not completed.</div><?php endif; ?>
            <p class="mb-1"><strong>Status:</strong> <span
                class="badge <?= shippingBadge($shipping['status']) ?>"><?= ucfirst(str_replace('_', ' ', $shipping['status'])) ?></span>
            </p>
            <p class="mb-1"><strong>Courier:</strong>
              <?= $shipping['courier'] ? htmlspecialchars($shipping['courier']) : '<span class="text-danger small">Not set</span>' ?>
            </p>
            <p class="mb-2"><strong>Tracking #:</strong>
              <?= $shipping['trackingNumber'] ? htmlspecialchars($shipping['trackingNumber']) : '<span class="text-danger small">Not set</span>' ?>
            </p>
            <hr>
            ...
            <?php if (!$formLocked): ?>
              <hr>
              <div class="alert alert-warning py-2 px-2 small mb-2"><i
                  class="bi bi-exclamation-triangle me-1"></i><strong>Required:</strong> Save courier &amp; tracking before
                updating status.</div>
              <form method="POST" class="mb-3" id="shippingDetailsForm">
                <div class="mb-2">
                  <select name="courier" id="courierSelect" class="form-select form-select-sm" required
                    onchange="syncShippingBtn()">
                ...
                <div class="input-group input-group-sm mb-2">
                  <input type="text" name="trackingNumber" id="trackingInput" class="form-control"
                    placeholder="Tracking number" value="<?= htmlspecialchars($shipping['trackingNumber'] ?? '') ?>"
                    required oninput="syncShippingBtn()">
                  <button type="button" class="btn btn-outline-secondary" onclick="generateTracking()"><i
                      class="bi bi-arrow-clockwise"></i></button>
                </div>
                <button type="submit" name="updateShippingDetails" class="btn btn-outline-primary btn-sm w-100"><i
                    class="bi bi-save me-1"></i>Save Details</button>
              </form>
              <form method="POST" class="d-flex align-items-center gap-2 flex-wrap">
                ...
                <button type="submit" name="updateShippingStatus" id="shippingStatusBtn" class="btn btn-primary btn-sm"
                  <?php if (empty($shipping['courier']) || empty($shipping['trackingNumber'])): ?>disabled
                    title="Save courier & tracking number first" <?php endif; ?>>
            ...
            <?php endif; ?>
          <?php else: ?>
            <p class="text-muted small mb-0"><i class="bi bi-info-circle me-1"></i>No shipping record yet. Confirm the
              order first.</p>
          <?php endif; ?>
        </div>
      </div>

      <div class="d-grid"><a href="orders" class="btn btn-secondary"><i class="bi bi-arrow-left me-1"></i>Back to
          Orders</a></div>
    </div>
  </div>
</section>
...
<script>
  ...
  function syncShippingBtn() {
    ...
    const btn = document.getElementById('shippingStatusBtn');
    if (!btn) return;
    const filled = courier && tracking;
    btn.disabled = !filled;
    btn.title = filled ? '' : 'Save courier & tracking number first';
  }

  // Run once on load to reflect server-side state
  document.addEventListener('DOMContentLoaded', syncShippingBtn);
</script>
